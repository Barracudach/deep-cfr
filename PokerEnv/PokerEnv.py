...
class PokerEnv:
    ...
    def _get_biggest_bet_out_there_aka_total_to_call(self):
        ...
        return max(max([p.current_bet for p in self.seats]),0)
    ...
    def _should_continue_in_this_round(self, all_non_all_in_and_non_fold_p, all_nonfold_p):
        ...
        # next_act_player=self._get_player_that_has_to_act_next()
        # if all_done_actions(all_nonfold_p) and self.current_player.current_bet - next_act_player.current_bet < self._get_current_total_min_raise():
        #     return False
