    ...
    def traverse_full_tree(self, state, env_wrapper, depth=0, parent_node=None, action_taken=None):
        ...
        for action in env_wrapper.legal_actions():
            ...
            if _done:
                if _rew_for_all[0]==0:
                    self.vvv=True
                    #print(f"Board:{_obs['board_cards']} Players:{_obs['players_cards']}")
                leaf_id = self.add_node(depth + 1, current_player, action_taken=action, reward=_rew_for_all[current_player],board=str(_obs['board_cards']))
