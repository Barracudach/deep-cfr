...
class MaybeRay:
    ...
    def init_local(self):
        if self.runs_distributed:
            ray.init(
                _redis_max_memory=min(10 ** 10, int(psutil.virtual_memory().total * 0.1)),
                object_store_memory=min(2 * (10 ** 10), int(psutil.virtual_memory().total * 0.4)),
            )
    ...
    def state_dict_to_torch(self, _dict, device):
        ...
        if self.runs_distributed:
            for k in list(_dict.keys()):
                if isinstance(_dict[k], np.ndarray):
                    # Создаем копию массива, чтобы сделать его записываемым
                    _dict[k] = np.array(_dict[k], copy=True)  # Делаем массив записываемым
    ...
    def grads_to_torch(self, g, device):
        if self.runs_distributed:  # if local, skip torch <--> numpy
            ...
            for k in list(g.keys()):
                if g[k] is not None:
                    copy = np.array(g[k], copy=True) 
                    g[k] = torch.from_numpy(copy)
